...
// It returns the controller and an error if any
// Parameters:
//   clients - the clients for the controller
...
//
// -- Doc autogenerated on 2022-05-12 20:48:56.267584 --
...
// It returns an error if the controller cannot be started
// Returns:
//   error: error if the controller cannot be started
// Example:
//   err := c.Activate()
//   if err != nil {
//       log.WithError(err)
...
// It returns an error if it fails to deactivate the controller
// Returns:
//   error: an error if it fails to deactivate the controller
// Example:
//   err := controller.Deactivate()
//   if err != nil {
//     log.Errorf("Failed to deactivate controller: %v", err)
//   }
//
// -- Doc autogenerated on 2022-05-12 20:48:56.267584 --
...
//   v, err := c.Version()
//   if err != nil {
//     log.Errorf("Error getting orchestrator version: %v", err)
//   }
//   log.Infof("Orchestrator version: %v", v)
//
// -- Doc autogenerated on 2022-05-12 20:48:56.267584 --
...
// reconcileTridentNotPresent reconciles the case where Trident is not present in the cluster.
// It returns an error if the reconciliation fails.
// Returns:
//   - nil if the reconciliation succeeds
//   - error if the reconciliation fails
// Example:
//   - reconcileTridentNotPresent()
//
// -- Doc autogenerated on 2022-05-12 20:48:56.267584 --
...
// reconcileTridentPresent reconciles the state of the Trident installation when the Trident deployment exists
// and is controlled by a CR.
// It returns an error if the reconcile failed.
// Parameters:
//   key - the key of the CR that triggered the reconcile
//   operatorCSIDeployments - the list of Trident CSI deployments in the namespace
//   deploymentNamespace - the namespace where Trident is installed
//   isCSI - true if Trident is installed as a CSI driver, false if it is installed as a Flexvolume driver
//   controllingCRBasedOnStatus - the CR that is currently controlling the Trident installation based on its status
// Returns:
//   error - an error if the reconcile failed
// Example:
...
//   err - error if any
// Example:
//   torcCR, err := updateTorcEventAndStatus(tridentCR, "debug message", "message", "status", "version", "namespace",
...
// cleanupTridentProvisioner cleans up the Trident provisioner
