...
// It returns a pointer to the controller and an error if one occurred
// Parameters:
//   clients - a pointer to the clients struct
// Returns:
//   *Controller - a pointer to the controller
//   error - an error if one occurred
// Example:
//   c, err := NewController(clients)
//   if err != nil {
//       log.Fatal(err)
//   }
//
// -- Doc autogenerated on 2022-05-12 18:38:47.645975 --
...
// It returns an error if the controller fails to start.
// Returns:
//   error: error starting the controller
// Example:
...
// Returns:
//   error: an error if the controller could not be stopped
// Example:
//   err := c.Deactivate()
//   if err != nil {
//     return err
...
// Returns:
//   string: the current version of the orchestrator
// Example:
...
// reconcileTridentNotPresent reconciles the state of the cluster when Trident is not installed.
//
// This method will identify a valid TridentOrchestrator CR and install Trident using that CR.
// It returns an error if it fails to install Trident.
// Returns:
...
// reconcileTridentPresent reconciles the Trident installation when Trident is present
//
// This function is called when Trident is present, i.e. Trident deployment(s) exist.
// This function identifies the controlling CR for the Trident installation and then
// performs reconcile based on the controlling CR.
// It returns an error if the reconcile fails.
// Parameters:
//   key: the key of the calling CR
//   operatorCSIDeployments: the list of Trident CSI deployments
//   deploymentNamespace: the namespace of the calling CR
//   isCSI: whether the calling CR is CSI based
//   controllingCRBasedOnStatus: the controlling CR based on status
// Returns:
//   error: error if reconcile fails
// Example:
...
// Parameters:
//   tridentCR - the TridentOrchestrator CR
//   debugMessage - the message to be logged as debug
//   message - the message to be logged as info
//   status - the status to be updated in the CR
//   version - the version of the CR
//   namespace - the namespace of the CR
//   eventType - the
// Returns:
//   torcCR - the updated TridentOrchestrator CR
//   err - error if any
// Example:
//   torcCR, err := c.updateTorcEventAndStatus(tridentCR, "", "", "", "", "", corev1.EventTypeNormal)
//
// -- Doc autogenerated on 2022-05-12 18:38:47.645975 --
func (c *Controller) updateTorcEventAndStatus(
...
// cleanupTridentProvisioner deletes the Trident provisioner CRD and all CRs.
// It returns an error if any of the operations fail.
// Returns:
//   - error: nil if all operations succeed, otherwise an error
// Example:
//   - err := c.cleanupTridentProvisioner()
//   - if err != nil {
//   -     log.Errorf("Failed to clean up Trident provisioner: %v", err)
//   - }
//
// -- Doc autogenerated on 2022-05-12 18:38:47.645975 --
func (c *Controller) cleanupTridentProvisioner() error {
...
// Returns:
//    true if the error is an API not found error
// Example:
//    err := ...
//    if isNotFoundError(err) {
//       // do something
